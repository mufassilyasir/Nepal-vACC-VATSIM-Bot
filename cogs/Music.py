...
class Player(wavelink.Player):
    ...
    async def add_tracks(self, ctx, tracks):
        ...
        if isinstance(tracks, wavelink.TrackPlaylist):
            ...
            embed = discord.Embed(title = "Queue", colour = discord.Color(0xff0000), timestamp = datetime.utcnow())
    ...
    async def choose_track(self, ctx, tracks):
        ...
        embed = discord.Embed(
            title=f"Hey {ctx.message.author.name}, I got some results. React to the message below to play/add that song. This message will expire in 30 seconds.",
            description=(
                "\n".join(
...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def connect_command(self, ctx, *, channel: t.Optional[discord.VoiceChannel]):
        ...
        embed = discord.Embed(title = f"I have connected to {channel.name}! What are you waiting? Play some songs.", colour = discord.Color(0xff0000))
    ...
    async def connect_command_error(self, ctx, error):
        if isinstance(error, AlreadyConnectedToChannel):
            embed = discord.Embed(title = "I am already connected to a voice channel, join me there. :slight_smile: ", colour = discord.Color(0xff0000))
            embed.set_footer(text=f"Error caused by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
        
        elif isinstance(error, NoVoiceChannel):
            embed = discord.Embed(title = "Ahh, I could not find you. Can you join a voice channel first? :sweat_smile:  Run this command again and I will find you than *hopefully*", colour = discord.Color(0xff0000))
            embed.set_footer(text=f"Error caused by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
    ...
    async def disconnect_command(self, ctx, channel = None):
        ...
        if player.is_connected == False:
            ...
            embed = discord.Embed(title = "You are not in a voice channel. To use that command join a voice channel.",colour = discord.Color(0xff0000))
            embed.set_footer(text=f"Error caused by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
            ...
            embed = discord.Embed(title = "Alright, I am disconnected now. See ya!", colour = discord.Color(0xff0000))
            embed.set_footer(text=f"Requested by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
    ...
    async def disconnect_command_error(self, ctx, error):
        if isinstance(error, AlreadyDisconnectedFromChannel):
            embed = discord.Embed(title = "I am already disconnected, how can I disconnect even more?", colour = discord.Color(0xff0000))
            embed.set_footer(text= f"Error caused by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
            


    @commands.command(name="play", aliases = ["p"], description = "To use Music bot you simply need to join any voice channel, run the command `?join` or simply use `?p` followed by any song to search or a direct link. Supported commands are `?play`, `?stop`, `?queue`, `?next`, `?previous`, `?repeat` and `?disconnect`. Run them to know what they do. :wink:" )
    async def play_command(self, ctx, *, query: t.Optional[str]):
        ...
        if query is None:
            ...
            embed = discord.Embed(title = "Back? Resuming song :arrow_forward: ", colour = discord.Color(0xff0000))
            embed.set_footer(text=f"Requested by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
    ...
    async def play_command_error(self, ctx, error):
        if isinstance(error, QueueIsEmpty):
            embed = discord.Embed(title = "Ahhh, I think I can't play **nothing**. Check the queue it's empty! :rolling_eyes: ", colour = discord.Color(0xff0000))
            embed.set_footer(text=f"Error caused by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
    ...
    async def pause_command(self, ctx):
        ...
        embed = discord.Embed(title = "Okay, I have paused the song :pause_button: ", colour = discord.Color(0xff0000))
        embed.set_footer(text=f"Requested by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
        await ctx.send(embed=embed)
    ...
    async def pause_command_error(self, ctx, exc):
        if isinstance(exc, PlayerIsAlreadyPaused):
            embed = discord.Embed(title = "Wait, I can't pause the player if it's already paused.... I am smort. :nerd: ", colour = discord.Color(0xff0000) )
            embed.set_footer(text=f"Error caused by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
    ...
    async def stop_command(self, ctx, channel=None):
        ...
        if player.is_connected == False:
            ...
            player.queue.empty()
            await player.stop()
            embed = discord.Embed(title = "Clearing up queue..... And stopped the current song. You may now add songs again. ", colour = discord.Color(0xff0000))
            embed.set_footer(text=f"Requested by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
    ...
    async def stop_command_error(self, ctx, error):
        if isinstance(error, AlreadyDisconnectedFromChannel):
            embed = discord.Embed(title = "Invite me to a voice channel first. :wink:", colour = discord.Color(0xff0000))
            embed.set_footer(text=f"Error caused by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
    ...
    async def next_command(self, ctx, channel = None):
        ...
        if player.is_connected == False:
            ...
            embed = discord.Embed(title = "Skipping the song... Moving on to the next one in the queue. :track_next: ", colour = discord.Color(0xff0000))
            embed.set_footer(text=f"Requested by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
    ...
    async def next_command_error(self, ctx, error):
        if isinstance(error, AlreadyDisconnectedFromChannel):
            ...
            embed = discord.Embed(title = "Could not do that, the queue is empty :cry: ", colour = discord.Color(0xff0000))
            embed.set_footer(text=f"Error caused by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
            
        elif isinstance(error, NoMoreTracks):
            embed = discord.Embed(title = "I wish I could skip... but there isn't any song in the queue. Why don't you add something? ", colour = discord.Color(0xff0000))
            embed.set_footer(text=f"Error caused by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
    ...
    async def previous_command(self, ctx, channel = None):
        ...
        if player.is_connected == False:
            ...
            await player.stop()
            embed = discord.Embed(title = "Like the song? Going back in time... :track_previous:  ", colour = discord.Color(0xff0000))
            embed.set_footer(text=f"Requested by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
    ...
    async def previous_command_error(self, ctx, error):
        if isinstance(error, AlreadyDisconnectedFromChannel):
            ...
            embed = discord.Embed(title = "I wish I could go back in time... but I cannot find any previous songs in my memory. :confused: ", colour = discord.Color(0xff0000))
            embed.set_footer(text=f"Error caused by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
    ...
    async def repeat_command(self, ctx,channel = None):
        ...
        if player.is_connected == False:
            ...
            embed = discord.Embed(title = "Set Repeat Mode", colour = discord.Color(0xff0000))
            embed.set_footer(text=f"Requested by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            ...
            try:
                ...
                if mode == "none":
                    embed = discord.Embed(title = "Repeat Mode set to None :no_entry:", colour = discord.Color(0xff0000))
                    embed.set_footer(text=f"Repeat mode set by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
                    await ctx.send(embed=embed)
                elif mode == "one":
                    embed = discord.Embed(title = "Repeat Mode set to repeat current track :one:", colour = discord.Color(0xff0000))
                    embed.set_footer(text=f"Repeat mode set by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
                    await ctx.send(embed=embed)
                elif mode == "all":
                    embed = discord.Embed(title = "Repeat Mode set to repeat all tracks in queue :repeat:", colour = discord.Color(0xff0000))
                    embed.set_footer(text=f"Repeat mode set by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
                    await ctx.send(embed=embed)
    ...
    async def queue_command_error(self, ctx, error):
        if isinstance(error, AlreadyDisconnectedFromChannel):
            ...
            embed = discord.Embed(title = "Hmmm, I could not find a song in the queue. Why don't you try command `?p` and search for the song or paste the link?", colour = discord.Color(0xff0000))
            embed.set_footer(text=f"Error caused by {ctx.message.author.name}", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
